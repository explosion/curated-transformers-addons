...
from curated_transformers.util.string import StringTransform, StringTransformations
from curated_transformers.models.hf_hub.conversion import process_hf_keys
...
def has_new_decoder_architecture(hf_config: Any) -> bool:
    return hf_config.get("new_decoder_architecture", False) or "n_head_kv" in hf_config


def convert_hf_config(hf_config: Any) -> FalconConfig:
    if has_new_decoder_architecture(hf_config):
        raise ValueError(
            "New decoder architecture is not supported by `FalconOldDecoder`/`FalconOldCausalLM`. "
            "Use `FalconDecoder` or `FalconCausalLM` from the `curated-transformers` package"
        )
